...
5. Based on `CHANGES.md`, update each workspace version following the rules of [semantic versioning](https://semver.org/), e.g.,
   `npm version minor -w @hztxi/cesium-engine --no-git-tag-version`. This includes `@cesium/sandcastle`.
...
> - Following the above rule, incrementing a workspace version will require an ensuing version bump in dependent workspaces—for example, `CHANGES.md` reports changes only in `@hztxi/cesium-engine`, but the required version increment for `@hztxi/cesium-engine` will also require a version increment for `@hztxi/cesium-widgets`.
...
4. Use `npm publish -w <WORKSPACE>` in the repository root (not the unzipped file directory) to publish the workspaces. Repeat this step for each updated workspace, in the following order:
   - `npm publish -w @hztxi/cesium-engine`
   - `npm publish -w @hztxi/cesium-widgets`
